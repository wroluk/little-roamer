...
import { EMBER_TRAILS, EMBER_START, EMBER_LOOKOUT, EMBER_FLOOR, EMBER_SIGNS, emberTrailSample } from '../src/game/northern-ember';
...
test('volcanic routes are dry and stay below a 29-degree grade', () => {
  for (const route of EMBER_TRAILS) for (let i = 0; i < route.points.length - 1; i++) {
...
test('volcanic clearings are level and the crater floor has ash handling', () => {
  for (const p of [EMBER_START, EMBER_LOOKOUT]) for (const dx of [-3, 0, 3]) for (const dz of [-3, 0, 3])
    assert.ok(Math.abs(sampledHeightAt(p.x + dx, p.z + dz) - p.y) < 0.1);
  assert.equal(northernSurfaceAt(EMBER_FLOOR.x, EMBER_FLOOR.z), 'ash');
  assert.equal(waterHeightAt(EMBER_FLOOR.x, EMBER_FLOOR.z), null);
  const a = generateNorthernChunk(5, 4), b = generateNorthernChunk(6, 4);
...
test('basalt columns stay outside driving lanes and signs belong to one chunk', () => {
  const signs = new Set<string>();
  let columns = 0;
  for (let cx = 5; cx <= 7; cx++) for (let cz = 3; cz <= 5; cz++) {
    const chunk = generateNorthernChunk(cx, cz);
    for (let i = 0; i < chunk.props.count; i++) {
      const type = NORTHERN_PROP_TYPES[chunk.props.type[i]];
      const x = chunk.props.x[i], z = chunk.props.z[i];
      if (type === 'basaltColumn') {
        assert.ok(emberTrailSample(x, z).distance >= 9.99);
        columns++;
      }
      if (type === 'emberSign') {
        const key = `${x},${z}`;
        assert.ok(!signs.has(key)); signs.add(key);
      }
    }
  }
  assert.ok(columns >= 10, 'several visible column groups should survive trail clearance');
  assert.deepEqual([...signs].sort(), EMBER_SIGNS.map(p => `${p.x},${p.z}`).sort());
});

test('the actual streamed vehicle completes each volcanic route with bounded streaming', async () => {
  for (const source of EMBER_TRAILS) for (const reversed of [false, true]) {
