...
import { PASS_TRAILS, PASS_START, PASS_LOOKOUT, PASS_BOWL } from '../src/game/northern-pass';
...
test('mountain routes are dry and stay below a 29-degree grade', () => {
  for (const route of PASS_TRAILS) for (let i = 0; i < route.points.length - 1; i++) {
...
test('mountain clearings are level and the frozen bowl has ice handling', () => {
  for (const p of [PASS_START, PASS_LOOKOUT]) for (const dx of [-3, 0, 3]) for (const dz of [-3, 0, 3])
    assert.ok(Math.abs(sampledHeightAt(p.x + dx, p.z + dz) - p.y) < 0.1);
  assert.equal(northernSurfaceAt(PASS_BOWL.x, PASS_BOWL.z), 'ice');
  assert.equal(waterHeightAt(PASS_BOWL.x, PASS_BOWL.z), null);
  const a = generateNorthernChunk(5, -6), b = generateNorthernChunk(6, -6);
...
test('the actual streamed vehicle completes each mountain route with bounded streaming', async () => {
  for (const source of PASS_TRAILS) for (const reversed of [false, true]) {
