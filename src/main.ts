...
import { WaterEffects } from './game/water-effects';
...
async function boot() {
  ...
  function createArea(area: Area) {
    ...
    try {
      ...
      const vehicle = new Vehicle(scene, world, area.spawn, area.climbingPower, area.surfaceAt);
      ...
      const waterEffects = new WaterEffects(scene, area.waterHeight);
      return { scene, world, vehicle, follow, waterEffects };
    } catch (error) {
      world.free();
      disposeScene(scene);
      throw error;
  ...
  let { scene, world, vehicle, follow, waterEffects } = createArea(area);
  ...
  async function travel(id: AreaId) {
    ...
    world.free();
    disposeScene(scene);
    ({ scene, world, vehicle, follow, waterEffects } = next);
    area = AREAS[id];
    presentArea();
  ...
  function frame(now: number) {
    ...
    if (mode === 'playing') {
      ...
      waterEffects.update(Math.min(elapsed, 0.05), vehicle);
